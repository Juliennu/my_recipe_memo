---
applyTo: "**/*.dart"
description: "Flutter/Dart coding rules"
---
...
### State Management
* **Riverpod:** Use `flutter_riverpod` as the primary state management solution.
  It provides a robust way to manage state, dependency injection, and data
  caching.
* **Code Generation:** Prefer using `riverpod_generator` for defining providers.
  It reduces boilerplate and ensures type safety. Use `@riverpod` annotation.
* **Providers:**
    * Use `Provider` for immutable values or objects (dependencies).
    * Use `StateProvider` for simple, mutable state.
    * Use `FutureProvider` for asynchronous data (like API calls).
    * Use `StreamProvider` for asynchronous streams.
    * Use `NotifierProvider` / `AsyncNotifierProvider` for more complex state
      logic using `Notifier` and `AsyncNotifier`.
* **ConsumerWidget:** Extend `ConsumerWidget` instead of `StatelessWidget` when
  you need to access providers.
* **ConsumerStatefulWidget:** Extend `ConsumerStatefulWidget` instead of
  `StatefulWidget` when you need both local state (lifecycle methods) and access
  to providers.
* **ref.watch vs ref.read:**
    * Use `ref.watch` inside `build()` to listen for changes.
    * Use `ref.read` inside callbacks (e.g., `onPressed`) to access the
      provider without listening.
...
* **Dark/Light Mode and Theme Toggle:** Implement support for both light and
  ...
  Riverpod provider) to allow for toggling between `ThemeMode.light`,
